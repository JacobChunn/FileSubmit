...
export default function DaySelector({
	...
	const [tempValue, setTempValue] = useState<string>((context.localSubordinateDetails as ExpenseDetailsExtended[])[index]['day'].toString());
	...
	const localDT = context.expenseDatestart;
    ...
    const applyClamping = () => {
        ...
        if (!isNaN(inputDay)) {
            ...
            setTempValue(context.localSubordinateDetails ? (context.localSubordinateDetails as ExpenseDetailsExtended[])[index]['day'].toString() : "");
        }
    };
	...
	const savedDay = context.dbSubordinateDetails[index] && (context.localSubordinateDetails as ExpenseDetailsExtended[])[index]['day'] === (context.dbSubordinateDetails as ExpenseDetailsExtended[])[index]['day'];
	const hideButtons = context.subordinateDetailsState == "signed";

	return (
		<div className="flex items-center justify-center px-2"
		>
            <input
                form={"form" + (context.selectedSubordinate as [number, "expense" | "timesheet", number])[2]}
                id="day"
                name={"EXD" + index + "[day]"}
                type="number"
                value={tempValue}
